...
#define METHOD(_RETURN, _NAME) JNIEXPORT _RETURN JNICALL Java_com_github_natanbc_lavadsp_natives_VolumeLibrary_##_NAME

#ifndef NO_CRITICALS
    #define CRITICAL_AVAILABLE true
    #define CRITICALNAME(_NAME) JavaCritical_com_github_natanbc_lavadsp_natives_VolumeLibrary_##_NAME
#else
    #define CRITICAL_AVAILABLE false
    #define CRITICALNAME(_NAME) FakeCritical_com_github_natanbc_lavadsp_natives_VolumeLibrary_##_NAME
#endif
...
#define SETTER(_NAME) \
    ...
    CRITICALMETHOD(void, process)(jlong instance, jint unused1, jfloat* input, jint inputOffset, jint unused2, jfloat* output, jint outputOffset, jint size) {
        ((Volume*)instance)->process(input, inputOffset, output, outputOffset, size);
    }
